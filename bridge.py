...
while True:
	...
	req = requests.get(url,  auth=HTTPBasicAuth('onos', 'rocks'))
        results_dict = json.loads(req.content)
	...
	for child_object in results_dict[elastic_index]:
		...
		url = "{0:s}/{1:s}/_doc/".format(elastic_url, elastic_index)
		req = requests.post(url, headers = json_header, data = json.dumps(child_object))
